...
{
  for (std::size_t i = 0; i < std::tuple_size_v<RGB>; ++i) {
    ptr[i] = rgb[i];
  }
}
...
{
  static_assert(std::tuple_size_v<RGB> == 3);
  ...
  , m_old_image(size.width(), size.height(), image_format)
{
  ...
  // screen descriptor
  {
    const uint8_t width = m_size.width();
    const uint8_t height = m_size.height();
    putc(width & 0xff);
    putc((width >> 8) & 0xff);
    putc(height & 0xff);
    putc((height >> 8) & 0xff);
...
QImage GifWriter::dither_image(const uint8_t* image, const GifWriter::GifPalette& palette)
{
  ...
  for (int yy = 0; yy < height; ++yy) {
    for (int xx = 0; xx < width; ++xx) {
      ...
      if (!m_old_image.isNull()) {
        if (rgb_from_ptr(&m_old_image.bits()[offset]) == rrggbb) {
...
QImage GifWriter::threshold_image(const uint8_t* image, const GifWriter::GifPalette& palette)
{
  ...
  for (uint32_t ii = 0; ii < num_pixels; ++ii) {
    ...
    if (last_frame && rgb_from_ptr(last_frame) == rgb_from_ptr(image)) {
      ...
      get_closest_palette_color(palette, { image[0], image[1], image[2] }, best_index, best_diff);
...
GifWriter::GifPalette::GifPalette(const QImage& last_frame, const QImage& next_frame, bool dither)
  : bit_depth(8), rgb(256, {0, 0, 0}), tree_split_elt(255, 0), tree_split(255, 0), m_dither(dither)
{
  ...
  {
    std::vector<uint8_t> destroyable_image(image_size);
    memcpy(destroyable_image.data(), next_frame.bits(), image_size);

    int num_pixels = (int)(width * height);
    if (!last_frame.isNull()) {
      num_pixels = pick_changed_pixels(last_frame.bits(), destroyable_image.data(), num_pixels);
    }

    const int last_elt = 1 << bit_depth;
    const int split_elt = last_elt/2;
    const int split_dist = split_elt/2;
    split(destroyable_image.data(), num_pixels, 1, last_elt, split_elt, split_dist, 1);
  }
